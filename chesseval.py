    ...
    def load_pieces(self):
        ...
        for color in colors:
            for piece in piece_names:
                key = color + piece
                self.piece_pixmaps[key] = QPixmap(f"chess_pieces/{key}.png").scaled(75, 75, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
...
class ChessGame(QWidget):
    ...
    def init_ui(self):
        ...
        self.setGeometry(100, 100, 550, 450)  # Increased width to accommodate new label

        layout = QHBoxLayout()

        left_layout = QVBoxLayout()
        self.move_quality_label = QLabel('Move: N/A')
        self.move_quality_label.setFixedSize(100, 30)
        self.move_quality_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        left_layout.addWidget(self.move_quality_label)
    ...
    def evaluate_position(self):
        ...
        if score is not None:
            ...
            self.evaluate_move_quality(score)
        else:
            mate = info['score'].relative.mate()
            self.eval_label.setText(f'Mate in {abs(mate)}')
            self.eval_bar.set_score(1000 if mate > 0 else -1000)
            self.move_quality_label.setText('Move: Brilliant' if mate > 0 else 'Move: Blunder')

        self.previous_score = score if score is not None else (1000 if mate > 0 else -1000)

    def evaluate_move_quality(self, new_score):
        if self.chess_board.board.turn == chess.WHITE:
            score_change = new_score - self.previous_score
        else:
            score_change = self.previous_score - new_score

        if score_change > 300:
            quality = "Brilliant"
        elif score_change > 200:
            quality = "Great"
        ...
        self.move_quality_label.setText(f'Move: {quality}')
    ...
    def reset_game(self):
        ...
        self.move_quality_label.setText('Move: N/A')
        self.previous_score = 0
